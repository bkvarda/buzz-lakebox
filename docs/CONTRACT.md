...
`config_schema` still drives the create-agent UI form (unchanged): `desktop/src/shared/api/types.ts` (`BackendProviderProbeResult`) declares the shape; `WhereToRunSection.tsx` → `ProviderConfigFields.tsx` walks `config_schema.properties` and renders one free-text input per property, seeded from `title`/`description`/`default`; `config_schema.required` gates the create-agent button; `coerceConfigValues` type-coerces the entered strings back (e.g. `"true"` → bool) before they're stored on the agent record. Those stored values are resent as `provider_config` on **every** redeploy. Current Buzz ignores string `enum`, but provider mode includes a dynamically discovered profile enum so a future renderer can expose a select without another provider contract change. It also includes the sorted names in the field description for today's text input.

For each `info` operation, provider mode runs `databricks auth profiles --skip-validate --output json` with a three-second timeout, parses stdout separately from stderr, and retains only validated profile names and HTTPS workspace hosts. It neither requests credentials nor validates/authenticates the profiles. Discovery failure falls back to the static schema and never makes provider discovery fail.

Convention (must not set `ok:false`; the `enum` and profile `default` are dynamic and omitted when discovery cannot supply them safely):

```json
{"ok":true,"name":"Databricks Lakebox","version":"<semver>","protocol_version":1,"description":"...","config_schema":{"type":"object","properties":{"profile":{"type":"string","title":"Databricks CLI profile","enum":["","EXAMPLE_PROFILE"],"default":"EXAMPLE_PROFILE","description":"Enter a discovered local Databricks CLI profile name, or leave empty for automatic selection. ..."},"inference_auth":{"type":"string","title":"Inference auth","default":"sandbox","description":"..."},"idle_timeout":{"type":"string","title":"Idle timeout","description":"..."},"mcp_config":{"type":"string","title":"Managed MCP servers (JSON)","description":"..."},"skills_config":{"type":"string","title":"Synchronized Databricks skills (JSON)","description":"..."}},"required":[]}}
```

We advertise the scalar `profile`, `inference_auth`, `idle_timeout`, `mcp_config`, and `skills_config` fields in `config_schema.properties`. An explicit deploy profile always bypasses discovery. For an empty profile, successful discovery prefers the baked default if present, otherwise selects the sole discovered profile, and refuses to guess among multiple names. Discovery failure or an empty result preserves the historical baked-default fallback. The MCP/skills fields are compact versioned JSON strings because the current Desktop renderer supports only scalar free-text inputs; their field copy directs operators through the CLI discovery/validation/probe path before pasting. `keep_workspace_pat`, `buzz_version`, `claude_adapter_version`, and `codex_adapter_version` stay expert-only — documented here in CONTRACT.md, not schema-advertised — since they're footguns better set deliberately in a hand-written payload than exposed as a free-text field in the desktop UI.
